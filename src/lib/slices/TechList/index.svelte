...
>
	...
	{#each slice.primary.items as { techname, tech_colour }}
		<div
			class=" tech-row mb-8 flex items-center justify-center gap-4 text-slate-700"
			aria-label={tech_colour || undefined}
		>
			{#each Array(15) as _, index}
				<span
					class="tech-item text-6xl font-extrabold uppercase tracking-tighter"
					style="color:{index === 7 && tech_colour ? tech_colour : 'inherit'}">{techname}</span
				>
				<span class="text-3xl"><IconCircle></IconCircle></span>
			{/each}
		</div>{/each}
</section>
