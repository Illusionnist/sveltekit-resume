		...
		>
			...
			>
				...
				{#each settings.data.nav_items as { label, link }}
					<li class="first:mt-8">
						<NavBarLink field={link} {label} {onLinkClick} type="mobile">{label}</NavBarLink>
					</li>
				{/each}
				{#if isFilled.link(settings.data.cta_link)}
					<Button linkField={settings.data.cta_link} label={settings.data.cta_label}></Button>{/if}
			</ul>
			<!-- desktop Nav -->
			<ul class=" relative z-50 hidden flex-row items-center gap-1 bg-transparent py-0 md:flex">
				{#each settings.data.nav_items as { label, link }}
					<li>
						<NavBarLink field={link} {label} {onLinkClick} type="desktop">{label}</NavBarLink>
					</li>
				{/each}
				{#if isFilled.link(settings.data.cta_link)}
